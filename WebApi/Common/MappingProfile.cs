...
namespace WebApi.Common
{
  public class MappingProfile : Profile // ": Profile" eklersek AutoMapper artık burayı bir config olarak görecek.
  {
    public MappingProfile()
    {
      ...
      CreateMap<Book,BookDetailViewModel>().ForMember(dest => dest.Genre, opt=>opt.MapFrom(src=>src.Genre.Name)); // Booku, BookDetailViewModel'e dönüştürür.
      CreateMap<Book,BooksViewModel>().ForMember(dest => dest.Genre, opt=>opt.MapFrom(src=>src.Genre.Name)); //destination içerisindeki dest.Genreyı şu şekilde maple, source üzerindeki GenreIdyi GenreEnumdan cast ederek bu idyi genreEnuma dönüştür, sornasında GenreEnum'ın string karşığını getir.
